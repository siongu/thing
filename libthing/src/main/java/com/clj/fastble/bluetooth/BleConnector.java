...
public class BleConnector {
    ...
    private com.clj.fastble.bluetooth.BleConnector withUUID(UUID serviceUUID, UUID characteristicUUID) {
    ...
    public com.clj.fastble.bluetooth.BleConnector withUUIDString(String serviceUUID, String characteristicUUID) {
    ...
    public boolean writeCharacteristic(byte[] data) {
        if (data == null || data.length <= 0) {
            BleLog.d("the data to be written is empty");
            return false;
        }

        if (mCharacteristic == null
                || (mCharacteristic.getProperties() & (BluetoothGattCharacteristic.PROPERTY_WRITE | BluetoothGattCharacteristic.PROPERTY_WRITE_NO_RESPONSE)) == 0) {
            BleLog.d("this characteristic not support write!");
            return false;
        }
        ...
        if (mCharacteristic.setValue(data)) {
            ...
            if (!mBluetoothGatt.writeCharacteristic(mCharacteristic)) {
                writeMsgInit();
                BleLog.d("gatt writeCharacteristic fail");
                return false;
            }
        } else {
            BleLog.d("Updates the locally stored value of this characteristic fail");
            return false;
        }
        return true;
    }
